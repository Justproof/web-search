...
// Additive migrations for databases created by an earlier version. Each is
// tried independently and a duplicate-column error is the expected no-op.
const MIGRATIONS = [
	"ALTER TABLE fetch_log ADD COLUMN abort_decision INTEGER",
	"ALTER TABLE blocklist ADD COLUMN session_id TEXT",
];
...
export const incrementSessionCounter = (sessionId: string): number => {
	...
	const row = db
		.prepare("SELECT web_tool_call_count FROM sessions WHERE session_id = ?")
...
// Every registrable-suffix candidate for a hostname, most specific first:
// a.b.evil.com -> [a.b.evil.com, b.evil.com, evil.com]. Blocking "evil.com"
// has to block its subdomains or the blocklist is trivially sidestepped.
const domainCandidates = (domain: string): string[] => {
	const parts = domain.toLowerCase().split(".").filter(Boolean);
	const out: string[] = [];
	for (let i = 0; i + 2 <= parts.length; i++) {
		out.push(parts.slice(i).join("."));
	}
	return out.length > 0 ? out : [domain.toLowerCase()];
};
...
export const isDomainBlocked = (
	...
	for (const candidate of domainCandidates(domain)) {
		const row = db
			...
			.get(candidate) as BlocklistEntry | undefined;
		if (!row) {
			continue;
		}
		if (row.expires_at && new Date(row.expires_at).getTime() < Date.now()) {
			db.prepare("DELETE FROM blocklist WHERE domain = ?").run(candidate);
			continue;
		}
		// Session-scoped entries only apply inside the session that added them.
		if (
			row.source === "session" &&
...
export const addToBlocklist = (
	...
	const db = getDb();
	db.prepare(
		`INSERT INTO blocklist (domain, reason, added_at, source, expires_at, session_id)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(domain) DO UPDATE SET reason = excluded.reason, source = excluded.source, expires_at = excluded.expires_at, session_id = excluded.session_id`,
	).run(
		entry.domain,
		entry.reason,
...
// How many fetches against this domain in this session ended in an abort
// verdict. Drives the FR-22.4 "repeated high-risk behaviour" auto-blocklist.
export const abortCountForDomain = (
	sessionId: string,
	domain: string,
): number => {
	const db = getDb();
	const row = db
       ...
       WHERE session_id = ? AND domain = ? AND abort_decision = 1`,
		)
		.get(sessionId, domain) as { n: number } | undefined;
	return row?.n ?? 0;
};
	...
	try {
		...
		db.transaction(() => {
           ...
           ON CONFLICT(domain) DO UPDATE SET reason = excluded.reason, source = 'user', expires_at = excluded.expires_at`,
				).run(entry.domain, entry.reason, entry.added_at, entry.expires_at);
			}
		})();
	} catch {
...
// Opportunistic retention. Without this, fetch_log / sessions / robots_cache
// grow without bound for the life of the install. Runs at most once a day and
// never touches the blocklist.
export const pruneOldRows = (retentionDays: number): void => {
	if (!Number.isFinite(retentionDays) || retentionDays <= 0) {
		return;
	}
	const last = getConfigValue("last_prune_at");
	const now = Date.now();
	if (last && now - new Date(last).getTime() < 24 * 3600 * 1000) {
		return;
	}
	const db = getDb();
	const cutoff = new Date(now - retentionDays * 24 * 3600 * 1000).toISOString();
	try {
		db.transaction(() => {
			db.prepare("DELETE FROM fetch_log WHERE fetched_at < ?").run(cutoff);
			db.prepare("DELETE FROM sessions WHERE started_at < ?").run(cutoff);
			db.prepare("DELETE FROM robots_cache WHERE fetched_at < ?").run(cutoff);
			db.prepare(
				"DELETE FROM blocklist WHERE expires_at IS NOT NULL AND expires_at < ?",
			).run(new Date(now).toISOString());
		})();
		setConfigValue("last_prune_at", new Date(now).toISOString());
	} catch {
		// Pruning is housekeeping — never let it break a fetch.
	}
};
