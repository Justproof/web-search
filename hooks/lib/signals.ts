...
import { countWrapperEscapes } from "./sanitise.ts";
...
export interface RiskTiersConfig {
	thresholds: {
		...
		fetch_log_retention_days: number;
	};
	signals: Record<string, SignalDefinition>;
	per_domain_overrides: Record<string, Partial<RiskTiersConfig["thresholds"]>>;
...
// Normalising a very large body costs real time inside a 3s budget. Bodies
// beyond this already fire oversized_response, so the tail is not load-bearing.
const MAX_NORMALISE_CHARS = 4_000_000;
...
export const computeSignals = (ctx: SignalContext): SignalResult => {
	...
	// injection_phrase — matched against the normalised body, not the raw one.
	// Raw matching was defeated by a single zero-width character inside the
	// phrase (which also stayed under zero_width_chars_max) or by swapping one
	// Latin letter for its Cyrillic twin.
	const haystack = normaliseForMatching(
		ctx.body.length > MAX_NORMALISE_CHARS
			? ctx.body.slice(0, MAX_NORMALISE_CHARS)
			: ctx.body,
	);
	const matchedPhrase = ctx.cfg.injection_phrases.find((p) =>
		haystack.includes(normaliseForMatching(p)),
	);
	if (matchedPhrase) {
	...
	// wrapper_escape_attempt — the body contains a tag that would close or forge
	// the agent-visible framing. There is no benign reason for fetched content
	// to contain </untrusted_source> or a <safe_research_summary> block.
	const wrapperEscapes = countWrapperEscapes(ctx.body);
	if (wrapperEscapes > 0) {
		fired.push("wrapper_escape_attempt");
		detail.wrapper_escape_count = wrapperEscapes;
	}

	// unicode_tag_chars — U+E0000-E007F encode ASCII invisibly and have no
	// legitimate use in web content. Any occurrence is Critical.
	if (ctx.tagCharCount > 0) {
		fired.push("unicode_tag_chars");
		detail.unicode_tag_char_count = ctx.tagCharCount;
	}

	// bidi_control_chars — trojan-source style reordering.
	const bidiCount = countMatches(ctx.body, BIDI_CONTROL_CHARS);
	if (bidiCount > 0) {
		fired.push("bidi_control_chars");
		detail.bidi_control_count = bidiCount;
	}
...
export const computeSimhash = (s: string): string => {
	...
	const tokens =
		s.toLowerCase().match(/\p{Ideographic}|[぀-ヿ가-퟿]|\p{L}{2,}/gu) ?? [];
	if (tokens.length === 0) {
		return "0".repeat(16);
	}
